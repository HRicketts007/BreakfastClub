...
import time
from meal_conversion import meal_defractorization
...
def generate_ics_calendar(meals, num_meals_perday, filename="meal_plan.ics"):
    ...
    start_date = today - timedelta(days=today.weekday() + 1)  # Adjust to the previous Sunday
    if today.weekday() == 6:  # If today is Sunday, keep it as start date
        start_date = today
    ...
    # Group meals into batches of `num_meals_perday`
    meal_groups = [meals[i:i + num_meals_perday * 3] for i in range(0, len(meals), num_meals_perday * 3)]

    # Loop through the grouped meals and assign them to days of the week
    for day_index, meal_group in enumerate(meal_groups):
        # Calculate the day of the corresponding date (Sunday-Saturday)
        event_date = start_date + timedelta(days=day_index)
        if event_date.weekday() > 6:  # Beyond Saturday
            break

        # Distribute meals across breakfast, lunch, and dinner
        for time_index, (meal_time, meal) in enumerate(zip(meal_times.keys(), meal_group)):
            event = Event()
            event.name = meal['meal_name']
            event.begin = datetime.combine(event_date, meal_times[meal_time])  # Set specific meal time
            event.description = f"Meal ID: {meal['meal_id']}\nReady in {meal['ready_in']} minutes.\nServings: {meal['servings']}\nMealURL: {meal['sourceUrl']}"
...
@app.route('/user/meal_plans', methods=['GET'])
@login_required
def get_user_plans():
    try:
...
@app.route('/meal_plan/<plan_id>/ingredients', methods=['GET'])
@login_required
...
def generate_calendar():
    ...
    generate_ics_calendar(meals=meals, num_meals_perday=3, filename=filename)
